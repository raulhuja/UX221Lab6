...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $sports_nutritionist_url The remote URL.
		 */
		public function __construct( $sports_nutritionist_url = '' ) {
			$this->remote_url = $sports_nutritionist_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $sports_nutritionist_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $sports_nutritionist_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $sports_nutritionist_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $sports_nutritionist_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $sports_nutritionist_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $sports_nutritionist_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $sports_nutritionist_format = 'woff2' ) {
			$this->font_format = $sports_nutritionist_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $sports_nutritionist_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $sports_nutritionist_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $sports_nutritionist_url, '/' ) ) {
				$sports_nutritionist_parsed_url = parse_url( $this->remote_url );
				return $sports_nutritionist_parsed_url['scheme'] . '://' . $sports_nutritionist_parsed_url['hostname'] . $sports_nutritionist_url;
			}

			return $sports_nutritionist_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $sports_nutritionist_url    The URL of the remote webfont.
	 * @param string $sports_nutritionist_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $sports_nutritionist_url, $sports_nutritionist_format = 'woff2' ) {
		$sports_nutritionist_font = new WPTT_WebFont_Loader( $sports_nutritionist_url );
		$sports_nutritionist_font->set_font_format( $sports_nutritionist_format );
		return $sports_nutritionist_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $sports_nutritionist_url, $sports_nutritionist_format = 'woff2' ) {
		$sports_nutritionist_font = new WPTT_WebFont_Loader( $sports_nutritionist_url );
		$sports_nutritionist_font->set_font_format( $sports_nutritionist_format );
		return $sports_nutritionist_font->get_url();
	}
}